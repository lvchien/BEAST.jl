``` 
    This script file aims at generating a triangular mesh for different geometries.
...
Create a mesh of an ice-cream cone of `radius` and `height`
...
Create a mesh of a square-based pyramid of `width` and `height`
...
Create a mesh of a star-based pyramid of star consisting of `nbofpoints` lying on `majorradius` and `nbofpoints` lying on `minorradius`, and `height`
