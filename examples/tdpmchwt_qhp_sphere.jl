``` 
    This script file aims at calculating the scattering of a transient electromagnetic plane-wave by a dielectric using qHP-TD-PMCHWT.
...
T0 = √8.25                                                                         # D/c with D the diameter of scatterer [second]

# for meshsize ∈ [0.4, 0.3, 0.2, 0.18, 0.16, 0.13, 0.12, 0.11, 0.1, 0.09]
    meshsize = 0.3
    # Computational mesh
    ...
    linform_9 = @discretise(H[k] + E[l], k∈Y⊗δ, l∈Y⊗δ)
    rhs_9 = BEAST.td_assemble(linform_9.linform, linform_9.test_space_dict)
    j_ext = [iNyx Zr; Zr iNyx] * rhs_9
    ...
    iZ01 = inv(Z01)
    jpmchwt13, ch1 = marchonintime(iZ01, Zxx1, rhs, Nt, convhist=true)
    ...
    iZ04 = inv(Z04)
    y, ch4 = marchonintime(iZ04, ZqHP, rhs_qhp, Nt; convhist=true)
    ...
    # open("sqtorus4holes_cdt_3.txt", "a") do io
    #     @printf(io, "%.4f %.4f %d %.4f %d \n", meshsize, cond(Z01), maximum(ch1) cond(Z04))
    # end;
# end
