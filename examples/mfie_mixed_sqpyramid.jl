``` 
    This script file aims at calculating the scattering of an electromagnetic plane-wave by a square-based pyramid PEC using the mixed MFIE discretization.
...
``` 
ref_mesh_size = 0.025
Γ_ref = meshsqpyramid(1.0, 3.0, ref_mesh_size)
...
mfie_ref = iN_ref * (0.5 * N_ref + K_ref)
rhs_efie_ref = assemble(@discretise -e[k] k∈X_ref)
rhs_mfie_ref = iN_ref * assemble(@discretise -h[k] k∈Y_ref)
...
### Print to file
open("sqpyramid.txt", "a") do io
    @printf(io, "%.3f %.3f %d %.12f %.3f %d %.12f \n", ref_mesh_size, cond(S_ref), ch_efie_ref.iters, norm_efie_ref, cond(mfie_ref), ch_mfie_ref.iters, norm_mfie_ref)
end; 
```
    end
```
...
### Computational mesh
for meshsize in [0.4, 0.35, 0.3, 0.25, 0.2, 0.185, 0.18, 0.16, 0.15, 0.135, 0.13, 0.12, 0.11, 0.1, 0.09, 0.08, 0.075, 0.07, 0.065, 0.055, 0.05, 0.04]
    # meshsize = 0.053
    Γ = meshsqpyramid(1.0, 3.0, meshsize)
    ...
    ### Print to file
    open("sqpyramid.txt", "a") do io
        @printf(io, "%.3f %.3f %d %.12f %.12f %.3f %d %.12f %.12f \n", meshsize, cond(Sxx), ch_efie.iters, err_efie_E, err_efie_M, cond(mfie), ch_mfie.iters, err_mfie_E, err_mfie_M)
    end; 
end
